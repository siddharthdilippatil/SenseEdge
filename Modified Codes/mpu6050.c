...
static int mpu6050_read_burst(struct i2c_client *client, u8 reg, u8 *buf, int len)
{
                          ...
                          unsigned int cmd, unsigned long arg)
{
    ...
    case MPU6050_SET_ACCEL_RANGE:
        i2c_smbus_write_byte_data(mpu->client, 0x1C, val);
        break;
    case MPU6050_SET_GYRO_RANGE:
        i2c_smbus_write_byte_data(mpu->client, 0x1B, val);
        break;
    default:
        mutex_unlock(&mpu->lock);
        return -EINVAL;
...
MODULE_DESCRIPTION("MPU6050 Multi-Device DT-Based Embedded Linux Driver");